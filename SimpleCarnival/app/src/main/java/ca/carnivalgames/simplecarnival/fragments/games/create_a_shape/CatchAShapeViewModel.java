...
public class CatchAShapeViewModel extends ViewModel {
   ...
   * @param cupView the cup animation used to detect whether the coin has been collected
   * @param coinX the X position of the coin before it falls
   * @param animationName The name of thsi animation to be added to the keyframe
   */
  ...
  private void updateState(AnimationEngine animationEngine,
    ...
    int newX = (int) Math.floor(Math.random() * (screenWidth - coinView.getWidth()));
    ...
    //Reset the animation
    animationEngine.removeKeyframe(animationName);
    setCoinAnimation(animationEngine, coinView, cupView, newX,animationName);
  }
  ...
  /**
   * Check collision between cup and coin
   * @param coin
   * @param cup
   * @return A boolean whether or not
   */
